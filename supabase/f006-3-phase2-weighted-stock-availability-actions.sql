...
declare v_action_authorized boolean := coalesce(current_setting('app.phase2_stock_action_authorized',true) = 'true',false);
begin
  if new.product_id is distinct from old.product_id or new.stock_code is distinct from old.stock_code
     ...
     or new.batch_line_no is distinct from old.batch_line_no then
    raise exception 'weighted_stock_action_or_correction_required';
  end if;
  if (new.status is distinct from old.status or new.order_id is distinct from old.order_id
      or new.order_item_id is distinct from old.order_item_id) and not v_action_authorized then
    raise exception 'weighted_stock_action_or_correction_required';
  end if;
...
declare v_old public.phase2_weighted_stock; v_new public.phase2_weighted_stock;
        v_previous text := current_setting('app.phase2_stock_action_authorized',true);
...
begin
  ...
  perform set_config('app.phase2_stock_action_authorized','true',true);
  update public.phase2_weighted_stock set status='manually_unlisted' where id=v_old.id returning * into v_new;
  perform set_config('app.phase2_stock_action_authorized',coalesce(v_previous,''),true);
