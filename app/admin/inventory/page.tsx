...
export default function AdminInventoryPage() {
  ...
  return <main className="admin adminOrdersPage inventoryAdminPage">
    <header className="adminTop ordersTop"><div><Link href="/admin">← 商品後台</Link><h1>🐟 今日魚貨／庫存</h1><p>直接修改價格、庫存與販售狀態。</p></div><button type="button" onClick={() => setShowCreate((value) => !value)}>{showCreate ? "關閉新增" : "＋ 新增今日魚貨"}</button></header>
    {showCreate && <form className="panel inventoryCreate" onSubmit={createProduct}><h2>新增今日魚貨</h2><div><label>商品名稱 *<input value={newProduct.productName} onChange={(event) => setNewProduct({ ...newProduct, productName: event.target.value })} /></label><label>第一個規格 *<input placeholder="例如：600–700g" value={newProduct.variantName} onChange={(event) => setNewProduct({ ...newProduct, variantName: event.target.value })} /></label><label>價格 *<input type="number" min={0} step={1} value={newProduct.price} onChange={(event) => setNewProduct({ ...newProduct, price: Number(event.target.value) })} /></label><label>庫存 *<input type="number" min={0} step={1} value={newProduct.inventory} onChange={(event) => setNewProduct({ ...newProduct, inventory: Number(event.target.value) })} /></label></div><label className="check"><input type="checkbox" checked={newProduct.processingEnabled} onChange={(event) => setNewProduct({ ...newProduct, processingEnabled: event.target.checked })} />啟用魚貨處理</label><button disabled={busyKey === "create"}>{busyKey === "create" ? "建立中…" : "建立商品與規格"}</button></form>}
