...
const newProductInitial = { productName: "", categoryId: "", processingEnabled: false, variantName: "", price: 0, inventory: 0, preorderEnabled: false, active: true };
...
export default function AdminInventoryPage() {
  ...
  const load = useCallback(async () => {
    const [productResult, variantResult, categoryResult, requestResult, movementResult] = await Promise.all([
    ...
    if (productResult.error || variantResult.error || categoryResult.error || requestResult.error) return setNotice("庫存、類別與配對資料載入失敗，請確認管理員權限。");
    ...
    setCategories((categoryResult.data || []) as ProductCategoryRecord[]);
  ...
  async function saveAllVariants(product: InventoryProduct) {
    ...
    const { error } = await supabase.rpc("admin_update_inventory_variants", {
      ...
      p_variants: product.variants.map((variant) => ({ id: variant.id, name: variant.name.trim(), price: variant.price, inventory: variant.inventory, preorder_enabled: Boolean(variant.preorder_enabled), active: variant.active }))
    });
    setBusyKey("");
  ...
  async function markSoldOut(product: InventoryProduct, variant: ProductVariant) {
    ...
    setBusyKey("");
    if (error) { changeVariant(product.id, variant.id, { inventory: previousInventory }, false); setNotice("設為現貨 0 件失敗，剩餘件數未變更。"); }
    else { setNotice(`${product.name}｜${variant.name} 現貨已設為 0 件。`); await load(); }
  }
  ...
  async function createProduct(event: React.FormEvent) {
    ...
    if (!newProduct.categoryId || !categories.some((category) => category.id === newProduct.categoryId && category.active)) return setNotice("請選擇有效的商品類別。");
    ...
    const { error } = await supabase.rpc("admin_create_inventory_product", {
      ...
      p_preorder_enabled: newProduct.preorderEnabled, p_category_id: newProduct.categoryId
    });
    setBusyKey("");
  ...
  return <main className="admin adminOrdersPage inventoryAdminPage">
    <header className="adminTop ordersTop"><div><Link href="/admin">← 商品後台</Link><h1>🐟 今日魚貨／剩餘件數</h1><p>以重量區間管理固定售價、現貨件數、預訂與販售狀態。</p></div><button type="button" onClick={() => setShowCreate((value) => !value)}>{showCreate ? "關閉新增" : "＋ 新增今日魚貨"}</button></header>
    {showCreate && <form className="panel inventoryCreate" onSubmit={createProduct}><h2>新增今日魚貨</h2><p className="inventoryGuidance">每個重量區間建立一個規格；重量皆為魚貨處理前重量。</p><div><label>商品名稱 *<input value={newProduct.productName} onChange={(event) => setNewProduct({ ...newProduct, productName: event.target.value })} /></label><label>商品類別 *<select required value={newProduct.categoryId} onChange={(event) => setNewProduct({ ...newProduct, categoryId: event.target.value })}><option value="">請選擇商品類別</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select></label><label>第一個重量區間 *<input placeholder="例如：150g～200g" value={newProduct.variantName} onChange={(event) => setNewProduct({ ...newProduct, variantName: event.target.value })} /></label><label>固定售價 *<input type="number" min={0} step={1} value={newProduct.price} onChange={(event) => setNewProduct({ ...newProduct, price: Number(event.target.value) })} /></label><label>現貨件數 *<input type="number" min={0} step={1} value={newProduct.inventory} onChange={(event) => setNewProduct({ ...newProduct, inventory: Number(event.target.value) })} /></label></div><label className="check"><input type="checkbox" checked={newProduct.preorderEnabled} onChange={(event) => setNewProduct({ ...newProduct, preorderEnabled: event.target.checked })} />接受預訂</label><label className="check"><input type="checkbox" checked={newProduct.processingEnabled} onChange={(event) => setNewProduct({ ...newProduct, processingEnabled: event.target.checked })} />啟用魚貨處理</label><button disabled={busyKey === "create"}>{busyKey === "create" ? "建立中…" : "建立商品與規格"}</button></form>}
    ...
    <section className="inventoryProductList">{visibleProducts.map((product) => {
      ...
      return <article className="panel inventoryProductCard" key={product.id}><header><div><h2>{product.name}</h2><span className={`inventoryState state-${inventoryProductState(product)}`}>{inventoryProductState(product) === "selling" ? "販售中" : inventoryProductState(product) === "sold_out" ? "售完" : "已下架"}</span>{match && <Link className="inventoryMatchIndicator" href={`/admin/matches?product=${product.id}`}>🔔 {match.requests.length} 人正在等</Link>}<small>{product.processing_enabled ? "🐟 已啟用處理" : "不提供處理"}</small></div><div><Link className="buttonLink secondaryAdminAction" href={`/admin/inventory/${product.id}`}>編輯商品</Link><Link className="buttonLink secondaryAdminAction" href={`/admin/processing?productId=${product.id}`}>處理設定</Link><button type="button" disabled={busyKey === product.id} onClick={() => toggleProduct(product)}>{product.status === "hidden" ? "重新上架" : "下架商品"}</button></div></header><fieldset className="inventoryVariants inventoryVariantsFieldset" disabled={batchBusy}>{product.variants.map((variant) => <div className="inventoryVariantRow" key={variant.id}><label>重量區間（處理前）<input placeholder="例如：150g～200g" value={variant.name} onChange={(event) => changeVariant(product.id, variant.id, { name: event.target.value })} /></label><label>固定售價<input type="number" min={0} step={1} value={variant.price} onChange={(event) => changeVariant(product.id, variant.id, { price: Number(event.target.value) })} /></label><label>現貨件數<input type="number" min={0} step={1} value={variant.inventory} onChange={(event) => changeVariant(product.id, variant.id, { inventory: Number(event.target.value) })} /></label><label className="inventoryActive"><input type="checkbox" checked={Boolean(variant.preorder_enabled)} onChange={(event) => changeVariant(product.id, variant.id, { preorder_enabled: event.target.checked })} />接受預訂</label><label className="inventoryActive"><input type="checkbox" checked={variant.active} onChange={(event) => changeVariant(product.id, variant.id, { active: event.target.checked })} />{variant.active ? "規格上架" : "規格下架"}</label><div><button type="button" className="secondaryAdminAction" disabled={busyKey === variant.id || variant.inventory === 0 || batchBusy} onClick={() => markSoldOut(product, variant)}>現貨歸零</button></div></div>)}</fieldset><footer className="inventoryBatchActions"><span aria-live="polite">{dirty ? "有尚未儲存的規格變更" : "全部規格已儲存"}</span><button type="button" disabled={!dirty || batchBusy} onClick={() => saveAllVariants(product)}>{batchBusy ? "儲存中…" : dirty ? "儲存全部規格" : "已儲存"}</button></footer></article>;
    })}</section>
