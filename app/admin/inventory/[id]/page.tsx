...
import AdminWeightedProductSettings from "@/components/admin-weighted-product-settings";
...
export default function InventoryProductDetailPage() {
  ...
  return <main className="admin adminOrdersPage inventoryDetailPage"><header className="adminTop ordersTop"><div><Link href="/admin/inventory">← 返回規格庫存</Link><h1>{product.name}</h1><p>商品內容與現貨設定分開儲存；單件秤重型使用長期價格級距。</p></div><Link className="buttonLink" href={`/admin/processing?productId=${product.id}`}>🐟 管理處理方式</Link></header><div className="inventoryDetailGrid"><AdminCatalogEditor id={id} onSaved={load} /><div><AdminWeightedProductSettings productId={id} onSaved={load}/>{product.inventory_mode!=="SINGLE_WEIGHTED"&&<form className="panel" onSubmit={addVariant}><h2>新增 legacy 規格</h2><label>重量區間（處理前）*<input placeholder="例如：150g～200g" value={newVariant.name} onChange={(event) => setNewVariant({ ...newVariant, name: event.target.value })} /></label><label>固定售價 *<input type="number" min={0} step={1} value={newVariant.price} onChange={(event) => setNewVariant({ ...newVariant, price: Number(event.target.value) })} /></label><label>現貨件數 *<input type="number" min={0} step={1} value={newVariant.inventory} onChange={(event) => setNewVariant({ ...newVariant, inventory: Number(event.target.value) })} /></label><label className="check"><input type="checkbox" checked={newVariant.preorder_enabled} onChange={(event) => setNewVariant({ ...newVariant, preorder_enabled: event.target.checked })} />接受預訂</label><label className="check"><input type="checkbox" checked={newVariant.active} onChange={(event) => setNewVariant({ ...newVariant, active: event.target.checked })} />規格上架</label><button disabled={busy}>{busy ? "新增中…" : "新增規格"}</button></form>}</div></div><section className="panel inventoryExisting"><h2>既有 legacy 規格</h2>{variants.map((variant) => <div key={variant.id}><strong>{variant.name}</strong><span>NT${variant.price.toLocaleString("zh-TW")}｜{variant.inventory > 0 ? `現貨｜剩 ${variant.inventory} 件` : variant.preorder_enabled ? "可預訂" : "已售完"}｜{variant.active ? "上架" : "下架"}</span></div>)}</section>{notice && <p className="notice centeredNotice" aria-live="polite">{notice}</p>}</main>;
}
