...
export default function AdminPage() {
  ...
  return (
    <main className="admin">
      <header className="adminTop"><div><Link href="/">← 返回商店</Link><h1>海鮮商品後台</h1><nav className="adminNavigation adminGroupedNavigation" aria-label="後台功能">
        <div><strong>訂單與客戶</strong><Link href="/admin/orders">🛒 訂單列表／今日摘要</Link><Link href="/admin/customers">👥 客戶</Link><Link href="/admin/requests">🔔 想找的魚</Link><Link href="/admin/matches">🔔 到貨配對{pendingMatchCount > 0 ? ` ${pendingMatchCount}` : ""}</Link></div>
        <div><strong>商品</strong><Link href="/admin#products">商品管理</Link><Link href="/admin/fish-catalog">🐟 魚種管理</Link><Link href="/admin/categories">🏷️ 商品類別</Link><Link href="/admin#products">處理方式（先選商品）</Link></div>
        <div><strong>庫存</strong><Link className="adminNavPrimary" href="/admin/weighted/quick-entry">＋ 今日魚貨快速上架</Link><Link href="/admin/weighted">單尾現貨管理</Link><Link href="/admin/inventory">規格數量庫存</Link></div>
        <div><strong>設定</strong><Link href="/admin/weighted/freshness">T+N 新鮮度規則</Link></div>
      </nav><p>{user}</p></div><button onClick={() => supabase.auth.signOut().then(() => setUser(""))}>登出</button></header>
      <section className="adminGrid">
        ...
        <section className="panel" id="products"><h2>商品管理</h2>{products.map((product) => <div className="manageRow" key={product.id}><div className="manageProduct"><div className="manageThumb">{primaryImage(product, galleryImages, process.env.NEXT_PUBLIC_SUPABASE_URL || "") ? <img src={primaryImage(product, galleryImages, process.env.NEXT_PUBLIC_SUPABASE_URL || "")!} alt={product.name} /> : <span>🦀</span>}</div><div><strong>{product.name}</strong><small>{productCategories.find((category) => category.id === product.category_id)?.name || "未分類"}｜{product.status === "available" ? "可購買" : product.status === "sold_out" ? "已售完" : "已隱藏"}｜{product.inventory_mode === "SINGLE_WEIGHTED" ? "單件秤重" : product.inventory_mode === "QUANTITY_VARIANT" ? "規格數量" : "未設定現貨模式"}</small></div></div><div className="manageActions"><Link className="buttonLink" href={`/admin/variants?productId=${product.id}`}>⚖️ 管理規格</Link><Link className="buttonLink" href={`/admin/processing?productId=${product.id}`}>🐟 處理設定</Link><Link className="buttonLink" href={`/admin/inventory/${product.id}`}>現貨設定／編輯商品</Link><button type="button" onClick={() => toggle(product)}>{product.status === "hidden" ? "上架" : "下架"}</button></div></div>)}</section>
      </section>
