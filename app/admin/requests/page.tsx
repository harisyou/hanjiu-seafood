...
export default function AdminRequestsPage() {
  ...
  const loadRequests = useCallback(async () => {
    const [requestResult, productResult, variantResult, catalogResult, orderResult] = await Promise.all([
      ...
      supabase.from("orders").select("id,fish_request_id,status")
    ]);
    if (requestResult.error || productResult.error || variantResult.error || catalogResult.error || orderResult.error) setNotice("需求與到貨資料載入失敗，請確認 F003-6 migration 與管理員權限。");
    else {
  ...
  const ordersByRequest = orders.reduce<Record<string, AdminOrder[]>>((result, order) => {
    if (order.fish_request_id) result[order.fish_request_id] = [...(result[order.fish_request_id] || []), order];
    return result;
  }, {});
