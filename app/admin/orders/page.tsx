  ...
  return <main className="admin adminOrdersPage">
    ...
    <section className="adminOrderList" aria-live="polite">{filteredOrders.length === 0 ? <div className="panel orderEmpty"><strong>沒有符合條件的訂單</strong><p>請調整搜尋或篩選條件。</p></div> : filteredOrders.map((order) => {
      ...
      return <article className="adminOrderCard" key={order.id}>
        ...
        <div className="orderCardItems">{order.order_items.map((item) => { const processing = processingSummary(item); return <div key={item.id}><strong>{item.product_name}｜{item.variant_name || "未指定規格"}｜{item.supply_type === "preorder" ? "預訂" : "現貨"} ×{item.quantity}</strong><span>處理：{processing.name}</span>{processing.extras.map((extra) => <span key={extra}>＋{extra}</span>)}{item.processing_note && <span>其他需求：{item.processing_note}</span>}</div>; })}</div>
