...
export default function AdminOrderDetailPage() {
  ...
  return <main className="admin orderDetailPage">
    ...
    <section className="orderDetailGrid">
      ...
      <article className="panel detailDelivery"><h2>配送資訊</h2><dl><div><dt>配送方式</dt><dd>{deliveryLabel(order.fulfillment)}</dd></div>{Object.entries(parsedNote.details).map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl>{order.status === "draft" && <p className="draftOrderNotice">草稿尚未確認配送與魚貨處理方式，也不代表已保留庫存。</p>}{parsedNote.customerNote && <section className="customerNote"><strong>📝 客人備註</strong><p>{parsedNote.customerNote}</p></section>}</article>
      <article className="panel detailStatuses"><h2>訂單狀態</h2><label>處理進度<select disabled={busy || order.status === "draft"} value={order.status} onChange={(event) => updateOrder({ status: event.target.value })}>{orderStatusOptions.map((option) => <option value={option.value} key={option.value}>{option.label}</option>)}{!orderStatusOptions.some((option) => option.value === order.status) && <option value={order.status}>{orderStatusLabel(order.status)}</option>}</select></label>{order.status === "draft" && <small>草稿需先補齊配送與處理資料，不能直接改為正式訂單狀態。</small>}<label>付款狀態<select disabled={busy} value={order.payment_status} onChange={(event) => updateOrder({ payment_status: event.target.value as PaymentStatus })}><option value="unpaid">未付款</option><option value="paid">已付款</option></select></label><div className="statusSnapshot"><span>{orderStatusLabel(order.status)}</span><span>{paymentStatusLabel(order.payment_status)}</span></div>{notice && <p className="notice" aria-live="polite">{notice}</p>}</article>
    </section>
