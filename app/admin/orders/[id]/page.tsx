...
export default function AdminOrderDetailPage() {
  ...
  return <main className="admin orderDetailPage">
    ...
    <section className="orderDetailGrid">
      <article className="panel detailCustomer"><header><div><small>建立時間</small><strong>{formatOrderTime(order.created_at, true)}</strong></div><div><small>總計</small><strong className="detailTotal">{orderTotal(order).toLocaleString("zh-TW")}</strong></div></header><h2>{order.customer_name}</h2><p><a href={`tel:${order.phone}`}>{order.phone}</a></p>{order.email && <p><a href={`mailto:${order.email}`}>{order.email}</a></p>}<div className="quickContact"><a className="buttonLink" href={`tel:${order.phone}`}>撥打電話</a>{order.email && <a className="buttonLink secondaryAdminAction" href={`mailto:${order.email}`}>寄 Email</a>}</div></article>
