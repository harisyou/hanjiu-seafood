...
export default function FishRequestForm() {
  ...
  async function submit(event: FormEvent) {
    ...
    const { data, error } = await supabase.rpc("create_fish_request", {
      p_customer_name: form.customerName, p_phone: form.phone, p_email: form.email || null,
  ...
  return <form className="fishRequestForm" onSubmit={submit} noValidate>
    <div className="fishRequestFields">
      ...
      <label>希望尺寸（選填）<input placeholder="例如：大尾一點、約1斤左右" value={form.size} onChange={(event) => setForm({ ...form, size: event.target.value })} /></label>
      <label>預算（選填）<input placeholder="例如：每尾 500 左右" value={form.budget} onChange={(event) => setForm({ ...form, budget: event.target.value })} /></label>
      <label>希望收到日期（選填）<input type="date" value={form.wantedBy} onChange={(event) => setForm({ ...form, wantedBy: event.target.value })} /></label>
      <label>用途（選填）<select value={form.purpose} onChange={(event) => setForm({ ...form, purpose: event.target.value })}><option value="">請選擇</option><option>家庭料理</option><option>聚餐</option><option>送禮</option><option>餐廳</option><option>其他</option></select></label>
      ...
      <label>手機號碼 *<input required type="tel" inputMode="tel" autoComplete="tel" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} /></label>
      <label className="fullField">Email（選填）<input type="email" autoComplete="email" placeholder="可填寫任何有效 Email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} /></label>
    </div>
    <fieldset className="notificationChoices"><legend>偏好通知方式</legend><label><input type="radio" name="notification" checked={form.channel === "line"} onChange={() => setForm({ ...form, channel: "line" })} />LINE（尚未綁定）</label><label><input type="radio" name="notification" checked={form.channel === "email"} onChange={() => setForm({ ...form, channel: "email" })} />Email</label><label><input type="radio" name="notification" checked={form.channel === "phone"} onChange={() => setForm({ ...form, channel: "phone" })} />簡訊／電話</label><small>LINE 尚未綁定時，韓九會依你留下的電話或 Email 聯絡；不會假裝能傳送 LINE 私訊。</small></fieldset>
    <label className="fishRequestNote">其他需求（選填）<textarea rows={5} placeholder={"例如：\n不要太小尾\n如果價格漂亮可以多拿\n希望適合清蒸\n餐廳每週可能固定需要"} value={form.note} onChange={(event) => setForm({ ...form, note: event.target.value })} /></label>
