...
export function validateInventoryValues(name: string, price: number, inventory: number) {
  ...
  if (!Number.isInteger(inventory) || inventory < 0) return "剩餘件數必須是大於或等於 0 的整數。";
  return "";
}

export function inventoryProductState(product: InventoryProduct) {
  if (product.status === "hidden") return "hidden";
  if (product.status === "sold_out" || product.variants.every((variant) => (variant.inventory === 0 && !variant.preorder_enabled) || !variant.active)) return "sold_out";
  return "selling";
}
