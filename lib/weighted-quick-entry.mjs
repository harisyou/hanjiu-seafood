...
export function gramsFromJinLiang(jin, liang) {
  if (!Number.isInteger(jin) || jin<0 || !Number.isFinite(liang) || liang<0 || liang>=16) return null;
  return Math.round(jin*600+liang*37.5);
}

export function jinLiangFromGrams(grams) {
  if (!Number.isInteger(grams) || grams<=0) return null;
  const jin=Math.floor(grams/600);
  const liang=Number(((grams%600)/37.5).toFixed(2));
  return {jin,liang};
}

export function jinLiangInputsForGrams(grams, jin, liang) {
  if (jin!=='' && liang!=='' && gramsFromJinLiang(Number(jin),Number(liang))===grams) return {jin,liang};
  const parts=jinLiangFromGrams(grams);
  return parts?{jin:String(parts.jin),liang:String(parts.liang)}:{jin,liang};
}
